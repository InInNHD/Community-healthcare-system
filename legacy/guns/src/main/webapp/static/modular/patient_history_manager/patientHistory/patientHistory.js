 ...
 * 导出单条就诊记录为Excel
 */
PatientHistory.exportOne = function () {
    if (this.check()) {
        window.open(Feng.ctxPath + "/patientHistory/export/" + PatientHistory.seItem.id);
    }
};
 ...
 * 按身份证号批量导出
 */
PatientHistory.exportByIdcard = function () {
    var idcard = $("#exportIdcard").val();
    if (!idcard) {
        Feng.info("请输入患者身份证号");
        return;
    }
 ...
 * 按日期范围批量导出
 */
PatientHistory.exportByDateRange = function () {
    var startDate = $("#startDate").val();
    var endDate = $("#endDate").val();
    if (!startDate || !endDate) {
        Feng.info("请输入开始日期和结束日期");
        return;
    }
    window.open(Feng.ctxPath + "/patientHistory/exportByDateRange?startDate=" + startDate + "&endDate=" + endDate);
};

/**
 * 打印单条就诊记录
 */
PatientHistory.printOne = function () {
    if (this.check()) {
        var url = Feng.ctxPath + "/patientHistory/print/" + PatientHistory.seItem.id;
